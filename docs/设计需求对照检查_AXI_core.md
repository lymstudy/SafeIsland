...
当前安全岛顶层已完成两版 RTL：
- **原版**：`safety_island_top.v` 为核心的 5 模块设计
- **融合版**（SafeIsland + ymliu）：`axi_safety_island_core.v` 为核心的 9 模块设计
...
- 已建立 ModelSim full TB、fault injection TB 和波形脚本。
- AXI out-of-order response 和 read interleaving 两个可选加分项已补充完成。
- 6 项 ASIL-D 安全增强已于 2026-06-25 完成设计与实现：CRC-16 E2E、心跳自检、KAT、TMR、Write-Verify、增强验证。

**当前验证状态：**
- 原版 ModelSim Full TB：17/17 PASS
- 原版 ModelSim Fault Injection：18/18 DETECTED（工程版保护率 100%）
- 增强版 Full TB：33+ 用例（含 CRC-16/TMR/KAT/heartbeat/write-verify），待最终回归
- 增强版 Fault Injection：21+ 用例（含 CRC/TMR/KAT 注错），待最终回归
- 融合版模块级测试：config_checker 9/9 PASS；s_axi_config 8 FAIL；data_fault 6 FAIL
- Icarus：22 用例中 17 PASS（77%），5 个受仿真器时序限制
...
| Fault case | 类型 | 期望结果 | 状态 |
|---|---|---|---|
| `run_kat_mismatch_fault` | KAT 预期值不匹配 | `safety_island_fault_detect` 置位 | 待最终回归 |
| `run_tmr_double_fault` | TMR state 双份故障 | `safety_island_fault_detect` 置位 | 待最终回归 |
| `run_e2e_crc16_mismatch` | CRC-16 E2E AR 通道故障 | `fault_detect` 置位 | 待最终回归 |
...
增强版预期：21/21 detected, undetected=0
...
- 融合版模块级测试存在失败：`s_axi_config` 8 个错误（CFG-007, CFG-009a~009d, CFG-010a, CFG-010b, CFG-012c），`data_fault` 6 个错误（DAT-002, DAT-004, FLT-001, FLT-002, FLT-007, FLT-009）。
- VCS/Verdi 模板已编写但未经 Linux 环境实测验证。
- 增强版 Full TB 和 Fault Injection 的最终回归尚未完成。

### 融合版（SafeIsland + ymliu）代码清单

| 模块 | 文件 | 说明 |
|------|------|------|
| 融合顶层 | `axi_safety_island_core.v` | 集成所有子模块 |
| 配置 Slave | `s_axi_config.v` | AXI4 Slave 配置 + Shadow/反码/锁 |
| 核心逻辑 | `safety_island_core_logic.v` | FSM 调度 + 数据流 + 反码保护 |
| 读引擎 | `safety_island_axi_read_engine.v` | AXI4 Master 读引擎 + Outstanding + CRC |
| 故障检测 | `safety_island_fault_detector.v` | Stuck 阈值检测 + Latent 检测 |
| 独立故障检测 | `fault_detector.v` | 融合版独立故障检测模块 |
| 故障状态管理 | `fault_status_manager.v` | 故障分类与状态管理 |
| 监控调度器 | `monitor_scheduler.v` | 扫描调度与 entry 遍历 |
| 读数据处理 | `read_data_processor.v` | Mask+OR 累加处理 |
| 配置校验 | `config_checker.v` | 独立配置合法性校验 |
| AXI Master 通道 | `axi_master_channel.v` | 融合版 AXI Master 核心 |
| 公共定义 | `axi_safety_island_pkg.vh` | 参数、地址、错误码定义 |

### 建议写入设计文档的 AoU

1. 监控 AXI read response 的 RID 必须匹配该 read engine 已发起且未完成的 ARID。
2. 外部对接模块必须为每个 R beat 生成校验位（CRC-8 或 CRC-16）。
   - CRC-8：校验对象 `{RID,RDATA,RRESP,RLAST}`，多项式 `0x07`，初值 `0x00`
   - CRC-16：校验对象 `{AR_SIGNATURE,RID,RDATA,RRESP,RLAST}`，多项式 `0x1021`，初值 `0xFFFF`；AR_SIGNATURE 覆盖 `{ARID,ARADDR,ARLEN,ARSIZE,ARBURST}`
...
1. 🔴 修复融合版模块级测试失败（s_axi_config 8 errors, data_fault 6 errors）
2. 🔴 运行增强版 Full TB (33+) 和 FI (21+) 最终 ModelSim 回归
3. 🔴 Linux 环境 VCS 仿真实测
4. 🟡 补全安全性理论分析文档（失效模型分析）
5. 🟡 ISO 26262 SPFM/LFM 正式计算与证明
6. 🟡 代码行覆盖率统计
7. 🟢 扩展认证级 fault campaign：自动遍历全网表/全寄存器 bit，统计 SPFM/LFM 所需覆盖率
8. 🟢 加 AXI protocol assertion 或 formal lint，证明握手、RID 匹配、CRC 校验和 burst 行为
9. 🟢 补 ZOIX 环境脚本：在有 ZOIX 的环境中执行错误仿真
